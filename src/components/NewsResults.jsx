...
const NewsResults = () => {
  ...
  const { getNewsResult, newsResult, loading, searchTerm, setSearchTerm } = useResult()
    ...
    <div className="results-container">
      ...
      <nav className="result-type-container">
          <div className="result-type">
            <Link to={"/search"}> <FcSearch size={"1em"} /> All</Link>
            <Link to={"/images"}> <FaRegImage size={"1em"} /> Images</Link>
            <Link to={"/news"} style={{borderBottom: "3px solid #8ab4f8"}}> <BiNews size={"1em"} /> News</Link>
          </div>
      </nav>
      ...
      <div className="result-links-container">
        <p style={{fontSize: "20px", margin: "0 0 20px 0"}}>News about {searchTerm}</p>
        {newsResult && newsResult.value.map(res => {
          return (
